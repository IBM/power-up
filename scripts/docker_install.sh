...
if [[ $ipv4_forwarding == "net.ipv4.conf.all.forwarding = 0" ]]; then
    ...
    if [ "$restart_docker" = true ]; then
        ...
        MESSAGE="WARNING: Docker service needs to be restarted to enable "
        MESSAGE+=" IPV4 forwarding!"
        add_message $MESSAGE
    fi

    ipv4_forwarding=$(/sbin/sysctl $net_ipv4_conf)
    if [[ $ipv4_forwarding == "net.ipv4.conf.all.forwarding = 0" ]]; then
        MESSAGE="ERROR: Unable to enable IPV4 forwarding! Ensure "
        MESSAGE+="'/sbin/sysctl $net_ipv4_conf' is set to '1'"
        add_message $MESSAGE
    fi
fi
